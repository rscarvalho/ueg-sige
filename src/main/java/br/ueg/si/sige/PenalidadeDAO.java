...
public class PenalidadeDAO {
    ...
    public Penalidade buscaPorCodigo(int codigo){
        ...
        try{db.prepareStatement("SELECT * FROM penalidades WHERE cd_penalidade=?");
        db.setInt(1, codigo);
        ...
        if (rs.next()){
            ...
            penalidade.setMatricula(new MatriculaDAO().buscaPorCodigo(rs.getInt("cd_matricula")));
        }
        rs.close();
        db.closeConnection();
        return penalidade;
    ...
    public Set buscaParametrizada(Map campos){
        ...
        if (campos.get("aluno")!=null){
            query += " AND cd_penalidade in(SELECT cd_penalidade FROM " +
                    ...
                    "=m.cd_matricula AND m.cd_aluno="+((Aluno)campos.get("aluno")).getCodigo()+")";
        }
        ...
        try{
            ...
            if (rs.next()){
                ...
                do{
                    Penalidade penalidade = this.buscaPorCodigo(rs.getInt("cd_penalidade"));
