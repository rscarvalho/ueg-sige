...
public class UsuarioDAO {
	public static Usuario buscaPorLoginSenha(String login, String senha) {
		...
		try {
			db.prepareStatement("SELECT cd_usuario FROM usuarios WHERE login = ? AND senha = encode('"
					+ senha + "','base64')");
	...
	public static Usuario buscaPorCodigo(int codigo) {
		...
		try {
			...
			if (rs.next()) {
				...
				Entidade entidade = EntidadeDAO.buscaPorCodigo(rs
						.getInt("entidade"));
				if (entidade != null) {
	...
	public static ArrayList<Usuario> buscaPorLogin(String login) {
		...
		try {
			...
			if (rs.next()) {
				...
				do {
					Usuario usuario = new Usuario();
					Entidade entidade = (Entidade) EntidadeDAO
							.buscaPorCodigo(rs.getInt("entidade"));
					if (entidade != null) {
						usuario.setEntidade(entidade);
					} else {
	 ...
	 * @return ArrayList
	 */
	public static ArrayList<Usuario> buscaPorNome(String nome) {
		SigeDataBase db = new SigeDataBase();
	...
	public static ArrayList<Usuario> buscaParametrizada(
			Map<String, Object> campos) {
		SigeDataBase db = new SigeDataBase();
		String query = "SELECT cd_usuario FROM permissao_por_usuario WHERE lower(nome_usuario)"
	...
	public static boolean salvarAlteracoes(Usuario usuario) {
		...
		try {
			...
			if (usuario.getSenha() != null) {
				query += ", senha=encode('" + usuario.getSenha()
						+ "','base64')";
			}
			...
			db.prepareStatement("INSERT INTO permissao_usuario(cd_usuario, cd_permissao)"
					+ " VALUES(?,?)");
	...
	public static boolean incluir(Usuario usuario) {
		...
		String query = "INSERT INTO usuarios(cd_usuario,cd_entidade,nome_usuario,cpf,login,senha,ativo)"
				+ " VALUES(default, ?, ?, ?, ?, encode( ?,'base64'), ?)";

		try {
			Statement stmt = db.getConn().createStatement();
			ResultSet rs = stmt
					.executeQuery("SELECT last_value as codigo FROM usuarios_cd_usuario_seq");
			rs.next();
