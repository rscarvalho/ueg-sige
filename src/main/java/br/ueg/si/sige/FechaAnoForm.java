...
public class FechaAnoForm extends HttpServlet {
	private static final long serialVersionUID = 9098100531609768185L;
	...
	private void fechaAno(HttpServletRequest request,
		...
		if (request.getMethod().equalsIgnoreCase("get")) {
			...
			campos.put("anoLetivo", new Integer(usuario.getEntidade()
					.getAnoLetivo()));
			campos.put("Entidade", usuario.getEntidade());
			Set<Boletim> boletins = dao.buscaParametrizada(campos);

			for (Iterator<Boletim> iter = boletins.iterator(); iter.hasNext();) {
				Boletim boletim = (Boletim) iter.next();
				Map<Disciplina, float[]> disciplinas = new HashMap<Disciplina, float[]>();
				for (Iterator<ItemBoletim> iter1 = boletim.getItensBoletim()
						.iterator(); iter.hasNext();) {
					ItemBoletim item = (ItemBoletim) iter1.next();
					if (disciplinas.get(item.getDisciplina()) == null) {
						...
						// incrementa
						float[] notaFalta = (float[]) disciplinas.get(item
								.getDisciplina());
						notaFalta[0] += item.getNota();
						notaFalta[1] += item.getFaltas();
						disciplinas.put(item.getDisciplina(), notaFalta);
				...
				Set<Disciplina> indices = disciplinas.keySet();
				for (Iterator<Disciplina> iter1 = indices.iterator(); iter1
						.hasNext();) {
				...
				if (aprovado) {
					boletim.getMatricula()
							.getAluno()
							.setEscolaridade(
									boletim.getMatricula().getAluno()
											.getEscolaridade() + 1);
				}
				AlunoDAO.salvarAlteracoes(boletim.getMatricula().getAluno());
			}

			usuario.getEntidade().setAnoLetivo(
					usuario.getEntidade().getAnoLetivo() + 1);
			request.getSession().setAttribute("usuarioAcademico", usuario);
			EntidadeDAO.salvarAlteracoes(usuario.getEntidade());
	...
	private void showFormConfirmaDados(HttpServletRequest request,
			HttpServletResponse response) throws IOException, ServletException {
		request.getRequestDispatcher("confirma_fechar_ano.jsp").forward(
				request, response);
	}
