...
<body>
	...
	<form name="form1" method="post" id="form1"
		action='turmas?action=<c:out value="${action}" />'
		onsubmit="javascript:envia()">
		<input type="hidden" name="codigo"
			value='<c:out value="${turma.codigo}"/>'>
		<table width="0" border="0" cellspacing="0" cellpadding="0">
			<tr>
				<td>Escola:</td>
				<td><select name="entidade" id="entidade" id="entidade">
						<option value='<c:out value="${turma.entidade.codigo}" />'
							selected>
							<c:out value="${turma.entidade.nome}" />
						</option>
						<c:forEach items="${entidades}" var="entidade">
							<option value='<c:out value="${entidade.codigo}" />'>
								<c:out value="${entidade.nome}" />
							</option>
						</c:forEach>
				</select></td>
			</tr>
			<tr>
				...
				<td><select name="serie" id="serie">
						<option value='<c:out value="${turma.serie.codigo}" />' selected>
							<c:out value="${turma.serie.numero}" />
						</option>
						<c:forEach items="${series}" var="serie">
							<option value='<c:out value="${serie.codigo}" />'>
			...
			<tr>
				...
				<td><input name="literal" type="text" class="input"
					id="literal" value='<c:out value=" ${turma.literal}" />' size="3"></td>
			</tr>
			<tr>
				<td>Ano:</td>
				<td><select name="ano" id="ano" id="ano">
						<option value='<c:out value="${turma.ano}" />' selected>
							<c:out value="${turma.ano}" />
						</option>
			...
			<tr>
				<td colspan="2"><div align="center">
						...
						<input name="Reset" type="reset" class="input"
							value="Limpar Dados">
					</div></td>
			</tr>
		</table>
